...
const pick = a => a[Math.floor(Math.random() * a.length)];
  ...
  function doInteract(type) {
    ...
    chrome.runtime.sendMessage({ action: 'interact', type }, (response) => {
      ...
      switch (type) {
        case 'feed': setState('idle'); if (S.currentPet === 'shiba') { setDogState('eating'); setTimeout(() => { setDogState('idle'); }, 10000); } else if (S.currentPet === 'cat') { setCatState('eating'); setTimeout(() => { setCatState('idle'); }, 10000); } else if (S.currentPet === 'rabbit') { setRabbitState('eating'); setTimeout(() => { setRabbitState('idle'); }, 10000); } showBubble('*munch munch* 🍖 Yum!', 3500); spawn('food', 6); doAction('surprised', 500); break;
        case 'bath': setState('idle'); if (S.currentPet === 'shiba') { setDogState('bathing'); setTimeout(() => { setDogState('idle'); }, 10000); } else if (S.currentPet === 'cat') { setCatState('bathing'); setTimeout(() => { setCatState('idle'); }, 10000); } else if (S.currentPet === 'rabbit') { setRabbitState('bathing'); setTimeout(() => { setRabbitState('idle'); }, 10000); } showBubble('*splash splash* 🛁 So clean!', 3500); spawn('sparkle', 8); break;
